...
public class VendedorController : Controller
{
    ...
    public VendedorController(IVendedorService service) => _service = service;
    ...
    public async Task<IActionResult> GetData([FromForm] DataTablesRequest request)
    {
        ...
        return Json(new DataTablesResponse<VendedorDto>
        {
            Draw = request.Draw,
    ...
    {
        var dto = id == 0 ? new VendedorDto() : await _service.GetByIdAsync(id) ?? new VendedorDto();
        return PartialView("_Form", dto);
    }

    [HttpPost]
    [IgnoreAntiforgeryToken]
    public async Task<IActionResult> Save([FromForm] VendedorDto dto)
    {
        try
        {
